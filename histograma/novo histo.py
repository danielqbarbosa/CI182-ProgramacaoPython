...
def calcula_tabela_frequencia(amostras):
    ...
    if len(amostras) == 0:
        ...
        k = int(round(1+ 3.3 * math.log10(len(amostras)), 0))
        print()
        ...
        h_rel = []  # inicia a lista da freq relativa
        print()
        print('Tabela de Frequencia')
        print('Classe|(Li |- Ls)|Frequencia|Acumulada|Relativa(%)')
        for c in range(0,k):
            classe = [int(num) for num in amostras if num in range(Li,Ls)]
            f_abs = len(classe)
            f_acum += len(classe)
        ...
        classe = [int(num) for num in amostras if num >= Li]
        if len(classe) > 0:
            f_abs = len(classe)
            f_acum += len(classe)
            f_rel = round(100*f_abs/len(amostras),1)
        ...
        for indice, valor in enumerate(h_rel):
